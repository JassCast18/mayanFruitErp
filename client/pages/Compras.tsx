...
import { dataStore, OrdenCompra } from "@/lib/data";
import {
  Plus,
  Search,
  Trash2,
...
const estadoColors: Record<string, { bg: string; text: string }> = {
  ...
  confirmado: { bg: "bg-blue-100 dark:bg-blue-900/20", text: "text-blue-700" },
  recibido: { bg: "bg-green-100 dark:bg-green-900/20", text: "text-green-700" },
...
interface OrdenCompraDetalle extends OrdenCompra {
...
export default function Compras() {
  ...
  const [ordenes, setOrdenes] = useState<OrdenCompraDetalle[]>(
    dataStore.getOrdenesCompra().map((o) => ({ ...o, mostrarDetalle: false }))
  );
  ...
  const stats = {
    ...
    confirmadas: ordenes.filter((o) => o.estado === "confirmado").length,
  ...
  return (
    <Layout title="Órdenes de Compra" subtitle="Gestión de compras a proveedores">
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {["todos", "pendiente", "confirmado", "recibido", "cancelado"].map(
              (estado) => (
                <button
                  key={estado}
                ...
                {orden.mostrarDetalle && (
                  <div className="p-6 bg-secondary/50 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {["pendiente", "confirmado", "recibido", "cancelado"].map(
                          (estado) => (
                            <button
                              key={estado}
                    ...
                    <div className="border-t border-border pt-4">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-medium text-foreground">
                            {orden.fechaEntregaEsperada || "No especificada"}
                          </p>
                        </div>
                        <div>
                          <p className="text-muted-foreground mb-1">
                          ...
                          <p className="font-medium text-foreground">
                            {orden.fechaEntregaReal || "Pendiente"}
                          </p>
                        </div>
                      </div>
                    </div>
